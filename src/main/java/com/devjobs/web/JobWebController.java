...
public class JobWebController {
  ...
  public String list(@RequestParam Optional<String> q, Model model) {
    List<Job> data = q.filter(s -> !s.isBlank()).map(jobs::search).orElseGet(jobs::list);
  ...
  public String create(
      @Valid @ModelAttribute("job") Job job, BindingResult result, Model m, RedirectAttributes ra) {
    if (result.hasErrors()) {
      m.addAttribute("workModes", WorkMode.values());
      m.addAttribute("types", JobType.values());
      return "jobs/form";
  ...
  public String update(
    ...
    Job existing =
        jobs.get(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
